...
║  Multi-Clone  ·  Batch Queue  ·  Auto-Delete  ·  Docker-Ready             ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
def extract_file_info(msg: Message) -> Optional[Dict[str, Any]]:
    ...
    if media == MessageMediaType.DOCUMENT:
        ...
        return {"file_id": g.file_id, "file_unique_id": g.file_unique_id,
                "file_type": "animation", "file_name": g.file_name or f"anim_{g.file_unique_id}.gif",
...
async def shorten_url_api(url: str) -> Optional[str]:
    if not config.SHORTENER_API or not config.SHORTENER_KEY:
        return None
    try:
    ...
    async def connect(self) -> AsyncIOMotorDatabase:
        ...
        for attempt in range(1, 6):
            try:
                ...
                self._client = AsyncIOMotorClient(config.MONGODB_URI, serverSelectionTimeoutMS=5000,
                                                   maxPoolSize=100, minPoolSize=10,
                                                   retryWrites=True, retryReads=True, w="majority")
                ...
                if attempt < 5: await asyncio.sleep(2 * attempt)
                else: raise RuntimeError(f"MongoDB connection failed after 5 attempts")

    async def close(self):
        if self._client: self._client.close(); self._client = None; self._db = None

    async def is_connected(self) -> bool:
        try:
            if self._client: await self._client.admin.command("ping"); return True
        except: pass
        return False

    @property
    def db(self) -> AsyncIOMotorDatabase:
        if self._db is None: raise RuntimeError("DB not connected. Call connect() first.")
        return self._db

    async def col(self, name: str) -> AsyncIOMotorCollection:
        return (await self.connect())[name]

database = Database()

# ═══════════════════════════════════════════════════════════════════════════════
# 6. SINGLE REPOSITORY (All-in-one operations)
# ═══════════════════════════════════════════════════════════════════════════════

class Repo:
    """Single unified repository for all MongoDB operations."""
    ...
    async def ensure_indexes():
        ...
        for coll, keys in collections.items():
            try:
                ...
                log.warning(f"Index error for {coll}: {e}")
        log.info("✅ All indexes created")

    # ── Users ──
    @staticmethod
    async def add_user(uid: int, clone: str = "main", username: str = "",
                       first: str = "", last: str = "") -> bool:
        col = await database.col("users")
    ...
    @staticmethod
    async def get_user(uid: int, clone: str = "main") -> Optional[dict]:
        col = await database.col("users")
        return await col.find_one({"clone_id": clone, "user_id": uid})

    @staticmethod
    async def ban_user(uid: int, clone: str = "main") -> bool:
        col = await database.col("users")
        r = await col.update_one({"clone_id": clone, "user_id": uid}, {"$set": {"is_banned": True}})
        return r.modified_count > 0

    @staticmethod
    async def unban_user(uid: int, clone: str = "main") -> bool:
        col = await database.col("users")
        r = await col.update_one({"clone_id": clone, "user_id": uid}, {"$set": {"is_banned": False}})
        return r.modified_count > 0

    @staticmethod
    async def is_banned(uid: int, clone: str = "main") -> bool:
        u = await Repo.get_user(uid, clone)
        return u.get("is_banned", False) if u else False

    @staticmethod
    async def count_users(clone: Optional[str] = None) -> int:
        col = await database.col("users")
        f = {"clone_id": clone} if clone else {}
        return await col.count_documents(f)

    @staticmethod
    async def get_all_users(clone: Optional[str] = None, limit: int = 10000) -> List[dict]:
        col = await database.col("users")
        f = {"clone_id": clone} if clone else {}
        return await col.find(f).limit(limit).to_list(length=limit)

    @staticmethod
    async def get_all_user_ids(clone: Optional[str] = None) -> List[int]:
        col = await database.col("users")
        f = {"clone_id": clone} if clone else {}
    ...
    @staticmethod
    async def inc_user_files(uid: int, clone: str = "main", amount: int = 1) -> bool:
        col = await database.col("users")
    ...
    # ── Files ──
    @staticmethod
    async def store_file(data: dict) -> str:
        col = await database.col("files")
        r = await col.insert_one(data)
        return str(r.inserted_id)

    @staticmethod
    async def get_file_by_token(token: str, clone: str = "main") -> Optional[dict]:
        col = await database.col("files")
        return await col.find_one({"clone_id": clone, "access_token": token, "deleted": False})

    @staticmethod
    async def get_file_by_id(fid: str, clone: str = "main") -> Optional[dict]:
        col = await database.col("files")
        return await col.find_one({"clone_id": clone, "file_id": fid, "deleted": False})

    @staticmethod
    async def count_files(clone: Optional[str] = None) -> int:
        col = await database.col("files")
        f = {"deleted": False}
    ...
    @staticmethod
    async def total_storage(clone: Optional[str] = None) -> int:
        col = await database.col("files")
        f = {"deleted": False}
        if clone: f["clone_id"] = clone
    ...
    @staticmethod
    async def soft_delete_file(fid: str, clone: str = "main") -> bool:
        col = await database.col("files")
    ...
    @staticmethod
    async def inc_file_dl(fid: str, clone: str = "main") -> bool:
        col = await database.col("files")
    ...
    # ── Batches ──
    @staticmethod
    async def create_batch(data: dict) -> str:
        col = await database.col("batches")
        r = await col.insert_one(data)
        return str(r.inserted_id)

    @staticmethod
    async def get_batch_by_token(token: str, clone: str = "main") -> Optional[dict]:
        col = await database.col("batches")
        return await col.find_one({"clone_id": clone, "access_token": token, "deleted": False})

    @staticmethod
    async def get_batch_by_id(bid: str, clone: str = "main") -> Optional[dict]:
        col = await database.col("batches")
        return await col.find_one({"clone_id": clone, "batch_id": bid, "deleted": False})

    @staticmethod
    async def update_batch_status(bid: str, clone: str, status: str) -> bool:
        col = await database.col("batches")
    ...
    @staticmethod
    async def update_batch_progress(bid: str, clone: str, sent: int, failed: int) -> bool:
        col = await database.col("batches")
        r = await col.update_one({"clone_id": clone, "batch_id": bid},
    ...
    @staticmethod
    async def inc_batch_dl(bid: str, clone: str = "main") -> bool:
        col = await database.col("batches")
        r = await col.update_one({"clone_id": clone, "batch_id": bid}, {"$inc": {"downloads": 1}})
        return r.modified_count > 0

    @staticmethod
    async def soft_delete_batch(bid: str, clone: str = "main") -> bool:
        col = await database.col("batches")
        r = await col.update_one({"clone_id": clone, "batch_id": bid},
    ...
    @staticmethod
    async def count_batches(clone: Optional[str] = None, status: Optional[str] = None) -> int:
        col = await database.col("batches")
        f = {"deleted": False}
        if clone: f["clone_id"] = clone
    ...
    @staticmethod
    async def get_batches(clone: Optional[str] = None, limit: int = 100, skip: int = 0) -> List[dict]:
        col = await database.col("batches")
        f = {"deleted": False}
        if clone: f["clone_id"] = clone
    ...
    @staticmethod
    async def get_expired_batches() -> List[dict]:
        col = await database.col("batches")
        now = datetime.now(timezone.utc).isoformat()
        return await col.find({"auto_delete_at": {"$ne": None, "$lte": now}, "deleted": False}).to_list(length=500)

    @staticmethod
    async def batch_total_storage(clone: Optional[str] = None) -> int:
        col = await database.col("batches")
        f = {"deleted": False}
        if clone: f["clone_id"] = clone
    ...
    @staticmethod
    async def get_settings(clone: str = "main") -> dict:
        col = await database.col("settings")
    ...
    @staticmethod
    async def set_settings(clone: str, updates: dict) -> bool:
        col = await database.col("settings")
        ...
        r = await col.update_one({"clone_id": clone}, {"$set": {"data": current}}, upsert=True)
        return True

    # ── Moderators ──
    @staticmethod
    async def add_mod(clone: str, uid: int, by: int = 0) -> bool:
        col = await database.col("moderators")
        try:
    ...
    @staticmethod
    async def remove_mod(clone: str, uid: int) -> bool:
        col = await database.col("moderators")
        r = await col.delete_one({"clone_id": clone, "user_id": uid})
        return r.deleted_count > 0

    @staticmethod
    async def is_mod(clone: str, uid: int) -> bool:
        col = await database.col("moderators")
        return await col.find_one({"clone_id": clone, "user_id": uid}) is not None

    @staticmethod
    async def get_mods(clone: str) -> List[dict]:
        col = await database.col("moderators")
        return await col.find({"clone_id": clone}).to_list(length=100)

    # ── Tokens ──
    @staticmethod
    async def create_token(clone: str, uid: int, token: str, days: int = 30) -> dict:
        col = await database.col("tokens")
    ...
    # ── Channels (force-sub / backup) ──
    @staticmethod
    async def add_channel(clone: str, cid: int, title: str = "", ctype: str = "force_sub") -> bool:
        col = await database.col("channels")
        try:
    ...
    @staticmethod
    async def remove_channel(clone: str, cid: int) -> bool:
        col = await database.col("channels")
        r = await col.delete_one({"clone_id": clone, "channel_id": cid})
        return r.deleted_count > 0

    @staticmethod
    async def get_channels(clone: str, ctype: str = "force_sub") -> List[int]:
        col = await database.col("channels")
    ...
    # ── Logs ──
    @staticmethod
    async def add_log(clone: str, uid: int, action: str):
        col = await database.col("logs")
        try:
    ...
    # ── Stats ──
    @staticmethod
    async def record_daily(clone: str):
        col = await database.col("stats")
        today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
        users = await Repo.count_users(clone)
        files = await Repo.count_files(clone)
        batches = await Repo.count_batches(clone)
        storage = await Repo.total_storage(clone) + await Repo.batch_total_storage(clone)
        await col.update_one(
    ...
    @staticmethod
    async def get_stats(clone: str, days: int = 7) -> List[dict]:
        col = await database.col("stats")
...
class DeliveryEngine:
    ...
    async def _execute(self, job: dict, wid: int):
        ...
        batch = await repo.get_batch_by_id(batch_id, clone)
        if not batch: return
        await repo.update_batch_status(batch_id, clone, "delivering")
        ...
        for idx, fm in enumerate(files):
            ...
            while retries > 0:
                try:
                    ...
                    sent += 1
                    await repo.update_batch_progress(batch_id, clone, sent, failed)
                    break
        ...
        if not cancelled:
            await repo.update_batch_status(batch_id, clone, "completed" if failed == 0 else "partial")
            await repo.inc_batch_dl(batch_id, clone)
            try:
            ...
            # Schedule auto-delete for all sent messages (approximate via progress msg)
            if auto_del > 0:
                auto_del_sched.schedule(clone, chat_id, progress_msg_id, auto_del)
        else:
            await repo.update_batch_status(batch_id, clone, "cancelled")
            try:
                await client.edit_message_text(
                    chat_id, progress_msg_id,
...
class CloneManager:
    ...
    def _register_handlers(self, client: Client, clone_id: str):
        ...
        async def start_c(c: Client, m: Message):
            ...
            # Check ban
            if await repo.is_banned(user.id, clone_id):
            ...
            settings = await repo.get_settings(clone_id)

            # Force sub
            fsubs = settings.get("force_subs", [])
            if not fsubs:
                fsubs = config.FORCE_SUB_CHANNELS
            if fsubs:
            ...
            if param.startswith("f_"):
                fid = param[2:]
                fd = await repo.get_file_by_id(fid, clone_id)
                if not fd:
                ...
                try:
                    ...
                    await repo.inc_file_dl(fid, clone_id)
                    if settings.get("auto_del_secs", 0) > 0:
            ...
            if param.startswith("b_"):
                bid = param[2:]
                batch = await repo.get_batch_by_id(bid, clone_id)
        ...
        async def done_c(c: Client, m: Message):
            ...
            await repo.add_log(clone_id, user.id, f"Created batch {batch_id[:16]}...")
        ...
        async def media_c(c: Client, m: Message):
            ...
            await repo.store_file(fd)
            await repo.inc_user_files(user.id, clone_id)
        ...
        async def cb_c(c: Client, q: CallbackQuery):
            ...
            # Admin callbacks
            if data.startswith("a_") or data.startswith("conf_") or data.startswith("cancel_") or data.startswith("settings_") or data.startswith("batch_") or data.startswith("cancel_dl:"):
    ...
    async def create_clone(self, token: str) -> Optional[dict]:
        ...
        try:
            ...
            data = {"clone_id": cid, "bot_token": token, "bot_username": me.username or f"bot_{me.id}",
                    "bot_id": me.id, "status": "active", "created_at": datetime.now(timezone.utc).isoformat()}
...
async def admin_callback_handler(c: Client, q: CallbackQuery, clone_id: str):
    ...
    # Dashboard
    if data == "admin_dashboard" or data == "a_admin":
    ...
    # Users
    if data == "a_users":
        users = await repo.get_all_users(limit=100)
        text = f"**👥 Users ({len(users)}):**\n\n"
        for u in users[:50]:
            text += f"• {u.get('first_name','?')} @{u.get('username','?')} `{u['user_id']}`\n"
        if len(users) > 50: text += f"\n... +{len(users)-50} more"
        await msg.edit_text(text, reply_markup=kb.back())
        return

    # Files
    if data == "a_files":
        tf = await repo.count_files()
        storage = await repo.total_storage()
    ...
    # Batches list
    if data == "a_batches":
        batches = await repo.get_batches(limit=10)
        if not batches:
            await msg.edit_text("📦 No batches found.", reply_markup=kb.back())
            return
    ...
    if data == "a_clones":
        ...
        for c in clones:
            ...
            text += f"{icon} `{c['clone_id']}` — @{c.get('bot_username','?')} [{s}]\n"
        text += "\n`/add_clone <token>` to add"
        await msg.edit_text(text, reply_markup=kb.back())
        return
    ...
    if data == "a_settings":
        await msg.edit_text(
            ...
            "Clone-specific settings via bot commands:\n"
            "`/set_start`, `/add_fsub`, `/remove_fsub`\n"
            "`/add_mod`, `/remove_mod`, `/set_autodel`",
            reply_markup=kb.back())
        return

    # Restart
    if data == "a_restart":
        await msg.edit_text("🔄 **Restart all bots?**", reply_markup=kb.confirm("restart_all"))
        return
    if data == "conf_restart_all":
        ...
        _start_time = time.time()
        await msg.edit_text("✅ **All bots restarted!**", reply_markup=kb.back())
        return
...
# ═══════════════════════════════════════════════════════════════════════════════
# 14. MAIN BOT COMMAND HANDLERS (registered separately)
# ═══════════════════════════════════════════════════════════════════════════════
...
async def backup_command(c: Client, m: Message):
    ...
    try:
        files = await (await database.col("files")).find({"deleted": False}).to_list(length=100000)
...
async def main():
    ...
    # Init main bot
    main_client = await clone_mgr.init_main()
    # Register admin commands on main
    ...
    # Register all other handlers for main
    clone_mgr._register_handlers(main_client, "main")
