#!/usr/bin/env python3
"""
Telegram File Store & Multi-Clone Bot (Fixed & Updated)
======================================================
...
# Admin IDs Handling (Safe parsing)
...
if _missing:
    raise SystemExit("❌ Missing environment variables: " + ", ".join(_missing))
...
async def del_fsub_cmd(cl, msg):
    ...
    p = msg.text.strip().split(maxsplit=1)
    if len(p) < 2:
        return await msg.reply_text("Usage: `/del_fsub @channel` or `/del_fsub all`")
    t = p[1].strip()
    if t == "all":
        r = await db.remove_fsub_channel("all")
        return await msg.reply_text("✅ All removed." if r else "⚠️ None to remove.")
...
async def add_clone_cmd(cl, msg):
    ...
    if len(p) < 3:
        return await msg.reply_text("Usage: `/add_clone <BOT_TOKEN> <MONGODB_URI>`")
    _, token, uri = p
    st = await msg.reply_text("🔄 Testing MongoDB…")
    ok, err = await Database.test_uri(uri)
    if not ok:
        return await st.edit_text(f"❌ MongoDB: {err}")
    ...
    try:
        ...
        return await st.edit_text(f"❌ Invalid token: {e}")
    try:
        cid = await db.register_clone(token, uri, msg.from_user.id)
    except Exception as e:
        return await st.edit_text(f"❌ Registration failed: {e}")
    try:
        await start_one_clone({"bot_token": token, "mongo_uri": uri})
    except Exception as e:
        await db.remove_clone(token)
        return await st.edit_text(f"❌ Clone start failed, rollback done: {e}")
...
@app.on_message(filters.private & ~filters.command(["start", "cancel", "add_fsub", "del_fsub", "add_clone"]))
async def broadcast_listener(cl, msg):
